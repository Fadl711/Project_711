...
<table class="w-[100%]" id="mainAccountsTable"  style="display: ;">
  ...
  <tbody>
   ...
   {{-- @if ($subAccount->Main_id==$mainAccount->sub_account_id)  --}}
        ...
        <td class="text-right tagTd">{{$mainAccount->typeAccount}}</td>
        <td class="text-right tagTd">{{$mainAccount->Type_migration}}</td>
        ...
        <td class="text-right tagTd">   <div class="flex items-center gap-1">
          ...
          <form action="{{route('products.destroy',$mainAccount->main_account_id)}}" method="POST">
            @csrf
            @method('DELETE')
