...
<table class="w-[100%]" id="mainAccountsTable" >
  ...
  <tbody>
   ...
   {{-- @if ($subAccount->Main_id==$mainAccount->sub_account_id)  --}}
        ...
        <td class="text-right tagTd">{{$mainAccount->main_account_id}}</td>
        <td class="text-right tagTd">{{$mainAccount->account_name}}</td>
        <td class="text-right tagTd">{{$mainAccount->Nature_account}}</td>
        @foreach ($TypesAccounts as $TypesAccount)
    ...
    <td class="text-right tagTd">
        <div class="flex items-center gap-1">
          <a  href="{{route('accounts.Main_Account.edit',$mainAccount->main_account_id)}}"  class="p-1  rounded-full  group transition-all duration-500  flex item-center">
          ...
          <button class="p-1 focus:outline-none  rounded-full  group transition-all duration-500  flex item-center" data-toggle="modal" data-target="#delete-modal-{{$mainAccount->main_account_id}}">
      ...
      <!-- Modal -->
      <div class="modal" tabindex="-1" role="dialog" id="delete-modal-{{$mainAccount->main_account_id}}">
        <div class="modal-dialog" role="document">
          <div class="modal-content bg-white rounded shadow-md">
            ...
            <div class="modal-footer">
              <button type="button" class="btn bg-blue-500 text-white" data-dismiss="modal">الغاء</button>
              <form action="{{route('accounts.Main_Account.destroy',$mainAccount->main_account_id)}}" method="POST">
                @csrf
                @method('DELETE')
