...
<div class="flex max-md:block p-1">
    <div class="min-w-[30%] border-x border-y border-orange-950 rounded-xl">
        <form method="POST" class="min-w-[30%]" id="productForm">
            @csrf
            <div class="flex">
                <div class="mb-1 p-1 w-full">
                    <label for="Product_name" class="btn">بحث عن المنتج</label>
            ...
            <div id="newProduc" class="col-span-6 sm:col-span-3 py-2">
                <button class="inputSale flex font-bold" type="submit">
            ...
            <div class="flex" id="printEndSave" style="display:">
                <div class="col-span-6 sm:col-span-3">
                    <button class="inputSale mt-2 flex" type="submit">
                        <svg class="w-6 h-5 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
    ...
    <div class=" min-w-full  rounded-lg  max-h-[500px] ">
        ...
        <tbody class="divide-y divide-gray-300">
            ...
            <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                <td class="tagTd  border-r border-r-orange-950">اسمنت</td>
                <td class="tagTd  border-r border-r-orange-950 ">154650565</td>
                <td class="tagTd ">5</td>
                <td class="tagTd">4,000</td>
                <td class="tagTd">4,500</td>
                <td class="tagTd">20,000</td>
...
<br><div id="results" class="results"></div>
...
$(document).ready(function() {
    ...
    $('#Quantity').on('input', function() {
        var quantity = parseInt($(this).val()); // الحصول على الكمية المدخلة
        var totalPrice = quantity * Purchase_price.val(); // حساب السعر ال��جمالي
        $('#Total').val(totalPrice); // ��ضافة السعر الجمالي ��لى حقل السعر ال��جمالي
       });
    ...
    function displayProductDetails(product) {
        ...
        //    Barcode      .val(product.barcod).trigger('change'); 
        //    product_name  .val(product.product_name).trigger('change');
           ...
           Categorie_id  .val(product.Categorie_id).trigger('change');
           Quantity      .val(product.Quantity).trigger('change');

        }

        $('#productInfo').html(`
            الاسم: ${product.product_name}<br>
            السعر: ${product.price}<br>
            الوصف: ${product.description}
        `);
        $('#productDetails').show(); // إظهار قسم تفاصيل المنتج
    }
});
</script>
  ...
  $(function() { 
    const form = $('#invoicePurchases'),
          ...
          invoiceField2 = $('#purchase_invoice_id2'), // حقل رقم الفاتورة
...
.done(function(response) {
    if (response.success) {
        ...
        const invoiceInput = $('#purchase_invoice_id');
        if (invoiceInput.length) {
            // تحديث قيمة حقل الفاتورة وتشغيل أي أحداث مرتبطة بتغييره
            invoiceField.val(response.invoice_number).trigger('change');
            invoiceField2.val(response.invoice_number).trigger('change');
...
.always(function() {
    ...
    // دعم التنقل بين الحقول باستخدام اختصارات لوحة المفاتيح
