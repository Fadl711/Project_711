...
@section('bonds')
    <div id="successAlert" style="display: none"
        class=" fixed top-4 right-4 bg-green-500 text-white px-6 py-4 rounded-lg shadow-lg" role="alert">
        <p></p>
    </div>
    <div id="errorMessage" style="display: none"
        class=" fixed top-4 right-4 bg-red-600 text-white px-6 py-4 rounded-lg shadow-lg" role="alert">
        <p></p>
    </div>
            ...
            <div class="flex gap-4">
                @foreach ($PaymentType as $index => $item)
                    <div class="flex items-center gap-2">
                        ...
                        <input type="radio" name="payment_type" id="payment_type_{{ $index }}"
                            value="{{ $item->value }}"
            ...
            <div class="flex container  shadow-md py-4 px-2 bg-white">
                <div class="w-[40%] max-sm:w-[30%]">
                    <div>
                        ...
                        <select dir="ltr" id="transaction_type" class="inputSale select2 input-field"
                            name="transaction_type">
                            @isset($ExchangeBond->transaction_type)
                                <option value="{{ $ExchangeBond->transaction_type }}"> {{ $ExchangeBond->transaction_type }}
                                </option>
                            @endisset
                ...
                <div class="text-gray-700 w-[50%] max-sm:w-[70%] ">
                    <div class="  text-center  ">
                        <div class="flex  " role="">
                            <div class=" text-center  ">
                                ...
                                <select dir="ltr" id="Currency" class="inputSale select2 input-field "
                                    name="Currency">
                                    @isset($currs)
                          ...
                          @endisset
                            ...
                            <div class="text-center">
                                ...
                                <input id="exchange_rate" class="inputSale" type="number" name="exchange_rate"
                                    value="{{ isset($ExchangeBond->exchange_rate) ? $ExchangeBond->exchange_rate : 1.0 }}">
                            </div>
                            <div class="">
                                <label for="Amount_debit" class="text-center">المبلغ</label>
                                <input name="Amount_debit" id="Amount_debit" type="text"
                                    @isset($ExchangeBond->Amount_debit)
                                value="{{ number_format($ExchangeBond->Amount_debit, 2, '.', ',') }}"
                            @endisset
                                    class="inputSale px-1" placeholder="0" required onblur="formatCurrency(this)">
                            </div>
                        </div>
                    </div>
                    <div class=" text-center ">
                        <label for="AccountReceivable" class="text-center "> حساب القبض </label>
                        <select name="AccountReceivable" id="AccountReceivable" dir="ltr"
                            ...
                            @isset($main_accounts)
                                ...
                                @foreach ($main_accounts as $mainAccount)
                                    <option
                                        @isset($ExchangeBond->Main_debit_account_id)
                    ...
                    <div class="text-center ">
                        ...
                        <select name="DepositAccount" id="DepositAccount" dir="ltr"
                            class="input-field select2 inputSale">
                            @isset($Debitsub_account_id)
                                <option value="{{ $Debitsub_account_id->sub_account_id }}">
                                    {{ $Debitsub_account_id->sub_name }}</option>
                            @endisset
                        </select>
                    </div>
                </div>
                        ...
                        <ul class="grid grid-cols-3  w-full py-1">
                            <li class="text-center">
                                <select name="PaymentParty" id="PaymentParty" class=" select2 inputSale" required>
                                    ...
                                    @isset($main_accounts)
                                        ...
                                        @foreach ($main_accounts as $mainAccount)
                                            <option
                                                @isset($ExchangeBond->Main_Credit_account_id)
                          ...
                          @endisset
                            ...
                            <li class=" text-center px-1">
                                <select name="CreditAmount" id="CreditAmount"
                                    class="block w-full select2 p-2 border rounded-md inputSale">
                                    @isset($Creditsub_account_id)
                                        <option value="{{ $Creditsub_account_id->sub_account_id }}">
                                            {{ $Creditsub_account_id->sub_name }}</option>
                                    @endisset
                                </select>
                            </li>
                            <li class="text-center">
                                <textarea class="inputSale" name="Statement" id="Statement" rows="3">
@isset($ExchangeBond->Statement)
{{ $ExchangeBond->Statement }}
@endisset
</textarea>
                            </li>
            ...
            <div class="flex  py-4 ">
                <div class="mx-10">
                    <input type="submit" id="submitButton"
                        @isset($submitButton) value="{{ $submitButton }}" @endisset
            ...
            <button type="reset"  class="text-purple-700 hover:text-white border border-purple-700 hover:bg-purple-800 focus:ring-4 focus:outline-none focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-purple-400 dark:text-purple-400 dark:hover:text-white dark:hover:bg-purple-500 dark:focus:ring-purple-900">
                ...
                <div class="mx-10" id="">
                    ...
                    <input type="text" id="payment_bond_id" name="payment_bond_id"
                        @isset($ExchangeBond->payment_bond_id)value="{{ $ExchangeBond->payment_bond_id }}"
                @endisset>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('Receip');
            const submitButton = document.getElementById('submitButton');

            form.addEventListener('submit', function(e) {
                // ✅ منع الإرسال المكرر في حال الصفحة تعيد تحميل نفسها قبل تعطيل الزر
                if (submitButton.disabled) {
                    e.preventDefault(); // يمنع الإرسال الثاني
                    return;
                }

                // ✅ تعطيل الزر وتغيير النص
                submitButton.disabled = true;
                submitButton.value = 'جاري الحفظ...';
            });
        });
        ...
        $(document).ready(function() {
            ...
            $('#submitButton').click(function(event) {
                ...
                if (isSubmitting) return; // ✅ منع الإرسال المكرر

                const DepositAccount = $('#DepositAccount').val();
                const CreditAmount = $('#CreditAmount').val();
                let Amount_debit = $('#Amount_debit').val().replace(/,/g, '');
                $('#Amount_debit').val(Amount_debit);

                var hasError = false;

                if (Amount_debit <= 0) {
                    $('#errorMessage').show().text('يجب أن يكون المبلغ أكبر من الصفر').fadeOut(5000);
                    hasError = true;
                }

                if (!DepositAccount) {
                    $('#errorMessage').show().text('يرجى إدخال حساب الإيداع').fadeOut(3000);
                    $('#DepositAccount').select2('open');
                    hasError = true;
                } else if (!CreditAmount) {
                    $('#errorMessage').show().text('يرجى إدخال حساب الدائن الفرعي').fadeOut(3000);
                ...
                // ✅ قفل الزر وتغيير النص
                isSubmitting = true;
                $('#submitButton').prop('disabled', true).val('جاري الحفظ...');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.error) {
                            ...
                            const invoiceField = response.payment_bond_id;
                            const url = `{{ route('receip.print', ':invoiceField') }}`.replace(
                                ':invoiceField', invoiceField);
                            window.open(url, '_blank', 'width=600,height=800');
                            window.location.href = '{{ route('Receip.create') }}';
                        }
                    },
                    error: function(xhr) {
                        if (xhr.status === 422) {
                    ...
                    complete: function() {
                        // ✅ إعادة تمكين الزر فقط عند الفشل
                        isSubmitting = false;
                        $('#submitButton').prop('disabled', false).val('حفظ');
                    }
                });
            });
        });
