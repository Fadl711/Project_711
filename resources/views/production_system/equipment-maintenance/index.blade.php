...
@section('productionSystem')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <form action="{{ route('equipment-maintenance.index') }}" method="GET"
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">كود
                            الصيانة</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">المعدة
                        </th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">خط
                            الإنتاج</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">النوع
                        </th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">التاريخ
                            المخطط</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">الحالة
                        </th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">العمليات
                        </th>
                    </tr>
                </thead>
                        ...
                        <tr class="@if ($maintenance->isOverdue()) bg-red-50 @endif">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ...
                                @if ($maintenance->status == 'scheduled')
                                    ...
                                    <form action="{{ route('equipment-maintenance.start', $maintenance) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        <button type="submit" class="text-green-600 hover:text-green-900 mx-1">بدء</button>
                                    </form>
                                @elseif($maintenance->status == 'in_progress')
                                    <button onclick="openCompleteModal({{ $maintenance->maintenance_id }})"
                                        class="text-green-600 hover:text-green-900 mx-1">إكمال</button>
                                @endif
                            </td>
                        </tr>
                    @endforeach
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                ...
                <form id="completeForm" method="POST" class="mt-4 space-y-4">
                    ...
                    <div>
                        <label for="parts_replaced" class="block text-sm font-medium text-gray-700">الأجزاء المستبدلة
                            (مفصولة بفواصل)</label>
