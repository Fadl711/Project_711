...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{ route('production-orders.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">
            إنشاء أمر جديد
        </a>
    </div>
  ...
  {{ \App\Models\ProductionOrder::getStatuses()[$order->status] }}                            </span>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{\Carbon\Carbon::parse($order->start_date)->format('Y-m-d') }}</td>

                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('production-orders.show', $order) }}" class="text-blue-600 hover:text-blue-900 mx-1">عرض</a>
                            <a href="{{ route('production-orders.edit', $order) }}" class="text-indigo-600 hover:text-indigo-900 mx-1">تعديل</a>
                            <form action="{{ route('production-orders.destroy', $order) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-900 mx-1" onclick="return confirm('هل أنت متأكد من الحذف؟')">حذف</button>
                            </form>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-500">لا توجد أوامر إنتاج</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
