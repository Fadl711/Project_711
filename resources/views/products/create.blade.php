...
<form id="products"  method="POST" class="border-b text-sm" >
        ...
        <div>
          ...
          <select name="Supplier_id" id="Supplier_id" dir="ltr" class="input-field w-full select2 inputSale" >
            <option selected value=""></option>
              ...
              @endisset
              value="{{$Supplier->sub_account_id}}">{{$Supplier->sub_name}}-{{$Supplier->sub_account_id}}</option>
               @endforeach
               @endisset
          </select>
      </div>
    ...
    @endauth
              <div class="grid grid-cols-2 gap-1 md:grid-cols-8 lg:grid-cols-8">
                <div  class="py-2">
                  <input type="button"  id="newProduc" name="newProduc" class="flex bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" @isset($editProduct)
                  value="{{$editProduct}}"
                  @endisset value="اضافة صنف">
                 
                </div>
            
        <div  class="py-2 mr-1 flex justify-between ml-1">
          <button type="button" id="Category" name="Category"  class="flex bg-green-500 hover:bg-green-700 text-white font-bold  py-2 px-4 rounded">
