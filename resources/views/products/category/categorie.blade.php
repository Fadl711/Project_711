        ...
        <table class=" rounded-xl ">
            <thead>
                <tr class=" ">
                    ...
                    <th scope="col" class="px-2 text-center ">اسم الوحدة</th>
                    <th scope="col" class="px-2 text-center ">رقم الوحدة</th>
                    <th scope="col" class="px-2 text-center ">سعر  الشراء </th>
                    <th scope="col" class="px-2 text-center ">سعر   البيع </th>
                    <th scope="col" class="px-2 text-center ">  العبوة </th>
                    <th scope="col" class="px-2  text-right">تعديل </th>
                </tr>
            </thead>
                ...
                <tr class="bg-white transition-all duration-500 hover:bg-gray-50 border-b-2">
                    ...
                    <td class="">
                      <div class="flex items-center gap-1">
                        ...
                        <button class="p-1 focus:outline-none  rounded-full  group transition-all duration-500  flex item-center" data-toggle="modal" data-target="#delete-modal-{{$cat->categorie_id}}">
                  ...
                  <!-- Modal -->
                  <div class="modal" tabindex="-1" role="dialog" id="delete-modal-{{$cat->categorie_id}}">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content bg-white rounded shadow-md">
                        <div class="modal-header">
                          <h5 class="modal-title text-lg font-bold">Delete Category</h5>
                          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                          </button>
                        </div>
                        <div class="modal-body">
                          <p class="text-gray-600 text-center font-bold">هل انت متاكد من حذف هذه الوحدة?</p>
                          <p class="text-gray-600 text-center">سوف يتم حذف المنتجات وكل العمليات المسجلة بهذا الوحدة</p>
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                          <form action="{{route('Category.destroy',$cat->categorie_id)}}" method="POST">
                            @csrf
                            @method('DELETE')
        ...
        <table class=" ">
            <thead>
                <tr class=" ">
                    ...
                    <th scope="col" class="px-1  text-center ">نوع العملية </th>
                    <th scope="col" class="px-1  text-center ">الكمية </th>
                    <th scope="col" class="px-1   text-right">تعديل </th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-300">
              @isset($uniquePurchase)
                  @foreach($uniquePurchase as $purchase)
                      @php
                          $transactionName = $purchase->transaction_type == 6 ? 'كمية الفتتاحية' : 'كمية مرحلة';
                      @endphp
                      <tr class="bg-white transition-all duration-500 hover:bg-gray-50" id="row-{{ $purchase->purchase_id }}">
                        <td class="tagTd">{{ $purchase->purchase_id }}</td>
                          <td class="tagTd">{{ $transactionName }}</td>
                          <td class="tagTd">{{ $purchase->quantity }}</td>
                          <td class="p-1 tagTd">
                              <div class="flex items-center gap-1">
                                <form class="destroy-form" action="{{ route('purchases.destroy', $purchase->purchase_id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد أنك تريد حذف هذا الصنف؟');">
                                  @csrf
                                  @method('DELETE')
                                  <button type="submit" class="btn btn-danger">Delete</button>
                              </form>
...
<script>
    ...
    $('.destroy-form').on('submit', function(e) {
        ...
        const form = $(this); // الحصول على النموذج الحالي
        ...
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        form.closest('tr').fadeOut(300); // إخفاء الصف
                    } else {
