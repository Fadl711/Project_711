        ...
        <table class=" ">
            <thead>
                <tr class=" ">
                    ...
                    <th scope="col" class="px-1  text-center ">اسم </th>
                    <th scope="col" class="px-1  text-center ">رقم </th>
                    <th scope="col" class="px-1  text-center ">سعر  الشراء </th>
                    <th scope="col" class="px-1  text-center ">سعر   البيع </th>
                    <th scope="col" class="px-1  text-center ">  العبوة </th>
            ...
            <tbody class="divide-y divide-gray-300">
              @isset($uniquePurchase)
                  @foreach($uniquePurchase as $purchase)
                      ...
                      @endphp
                      <tr class="bg-white transition-all duration-500 hover:bg-gray-50" id="row-{{ $purchase->product_id }}">
                          <td class="tagTd">{{ $transactionName }}</td>
                          <td class="tagTd">{{ $purchase->quantity }}</td>
                          <td class="tagTd">{{ $purchase->Purchase_price }}</td>
                          <td class="tagTd">{{ $purchase->Selling_price }}</td>
                          <td class="p-1 tagTd">
                              <div class="flex items-center gap-1">
                                <a href="'.route('products.edit', $product->product_id).'" class="p-1 rounded-full group transition-all duration-500 flex item-center">
                                                                <svg class="cursor-pointer" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  ...
                                  <a class="mt-[7px] focus:outline-none rounded-full group transition-all duration-500 flex item-center" href="{{ route('Category.create', $purchase->product_id) }}">
